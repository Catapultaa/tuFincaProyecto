...
const ResumenForm = ({ propiedadData, errors }) => {  // Eliminamos onEdit de los props
  ...
  const renderField = (label, value, required = true) => (
    <div className="py-2 border-b border-gray-100">
      <h2 className="text-sm font-semibold text-gray-600">{label}{required && '*'}</h2>
          ...
          <div className="mb-6 p-4 bg-red-50 rounded-lg border border-red-200">
            ...
            <ul className="mt-2 pl-7 space-y-1 text-red-600">
              {Object.entries(errors).map(([field, error]) => (
                <li key={field} className="list-disc">{error}</li>
              ))}
            </ul>
          </div>
        ) : (
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                  {propiedadData.archivos.map((archivo, index) => (
                    <div key={index} className="relative h-28 rounded-md overflow-hidden border border-gray-200">
                      {isVideo(archivo) ? (
                        <video
                          src={typeof archivo === 'string' ? archivo : URL.createObjectURL(archivo)}
