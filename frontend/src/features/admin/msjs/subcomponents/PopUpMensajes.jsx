...
const PopUpMensaje = ({
  ...
  const toggleEstado = async () => {
    try {
      ...
      const nuevoEstado =
        currentMensaje.gestion === "porLeer" ? "realizado" : "porLeer";

      // Enviar solo el ID del administrador
      const updateData = {
        gestion: nuevoEstado,
        administradorId: admin?.id, // <--- Cambiar a administradorId
      };

      const updated = await actualizarMensaje(currentMensaje.id, updateData);
